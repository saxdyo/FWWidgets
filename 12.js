...
async function loadTmdbByTheme(params = {}) {
  ...
  try {
    const cacheKey = `theme_v2_${theme}_${media_type}_${sort_by}_${min_rating}_${year}_${page}_${language}`;
    const cached = getCachedData(cacheKey);
    if (cached) return cached;

    console.log(`🎭 主题分类: ${theme}, 媒体: ${media_type}, 排序: ${sort_by}, 年份: ${year || "全部"}, 页码: ${page}`);

    // 主题到Genre ID的映射（电影/剧集分开）
    ...
    const genreMap = THEME_GENRE_MAP[theme];
    if (!genreMap) {
    ...
    const buildQueryParams = (type) => {
      ...
      if (sort_by === "popularity_desc") tmdbSort = "popularity.desc";
      else if (sort_by === "popularity_asc") tmdbSort = "popularity.asc";
      else if (sort_by === "release_date_desc") tmdbSort = type === "movie" ? "release_date.desc" : "first_air_date.desc";
      else if (sort_by === "release_date_asc") tmdbSort = type === "movie" ? "release_date.asc" : "first_air_date.asc";
      else if (sort_by === "vote_average_desc") tmdbSort = "vote_average.desc";
      else if (sort_by === "vote_count_desc") tmdbSort = "vote_count.desc";
      ...
      if (min_rating && min_rating !== "0") {
        queryParams["vote_average.gte"] = min_rating;
      }
      ...
      if (year) {
        ...
        if (type === "movie") {
          ...
          queryParams["first_air_date.gte"] = startDate;
          queryParams["first_air_date.lte"] = endDate;
        }
      }

      // 排除未上映内容（当按上映时间排序时）
      if (sort_by.includes("release_date") || sort_by.includes("air_date")) {
        const today = getBeijingDate();
        if (type === "movie") {
      ...
      return { endpoint, queryParams, type };
    };
    ...
    // 根据 media_type 执行查询
    if (media_type === "all") {
      const movieQuery = buildQueryParams("movie");
      const tvQuery = buildQueryParams("tv");

      const requests = [];
      if (movieQuery) requests.push(movieQuery);
      if (tvQuery) requests.push(tvQuery);

      const responses = await Promise.all(
        requests.map(async (req) => {
          try {
            console.log(`🎬 请求 ${req.type}: ${req.endpoint}`);
            const res = await Widget.tmdb.get(req.endpoint, { params: req.queryParams });
            return (res.results || []).map(item => {
              item.media_type = req.type;
              return item;
            });
          } catch (e) {
            console.error(`❌ ${req.type} 请求失败:`, e.message);
            return [];
          }
        })
      );
      ...
      const query = buildQueryParams(media_type);
      if (!query) {
        console.error(`❌ 不支持的媒体类型: ${media_type}`);
        return [];
      }

      console.log(`🎬 请求 ${media_type}: ${query.endpoint}`);
      const res = await Widget.tmdb.get(query.endpoint, { params: query.queryParams });
      allResults = (res.results || []).map(item => {
        item.media_type = media_type;
        return item;
      });
    }

    console.log(`📊 主题分类原始数据: ${allResults.length} 条`);

    // 数据过滤与转换
    const today = getBeijingDate();
    const filteredResults = allResults.filter(item => {
      // 必须有海报和标题
      if (!item.poster_path || !item.title || !item.title.trim()) return false;

      // 过滤无效海报
      if (item.poster_path.includes("null")) return false;

      // 日期有效性检查（排除未上映）
      const dateField = item.media_type === "movie" ? item.release_date : item.first_air_date;
      if (dateField && dateField > today) {
        console.log(`🚫 过滤未上映: ${item.title} (${dateField})`);
        return false;
      }
    ...
    console.log(`📊 过滤后数据: ${filteredResults.length} 条`);

    if (filteredResults.length === 0) {
      console.log("⚠️ 无有效数据，尝试回退获取");
      return await loadThemeFallback_v2(params);
    }

    // 转换为标准 WidgetItem
    const widgetItems = filteredResults.map(item => {
      const widgetItem = createWidgetItem(item);
      ...
      widgetItem.source = "TMDB主题分类";

      // 评分颜色标记
      const rating = parseFloat(widgetItem.rating) || 0;
      if (rating >= 8.0) widgetItem.ratingColor = "#FFD700";
      else if (rating >= 7.0) widgetItem.ratingColor = "#90EE90";
      else if (rating >= 6.0) widgetItem.ratingColor = "#FFA500";
      else widgetItem.ratingColor = "#FF6B6B";

      // 排序信息展示
      if (sort_by.includes("popularity")) {
        widgetItem.sortInfo = `热度 ${Math.round(item.popularity || 0)}`;
      } else if (sort_by.includes("vote_average")) {
        widgetItem.sortInfo = `评分 ${rating.toFixed(1)}`;
      } else if (sort_by.includes("release_date") || sort_by.includes("air_date")) {
        const date = item.media_type === "movie" ? item.release_date : item.first_air_date;
        if (date) widgetItem.sortInfo = `${date.substring(0, 4)}年`;
      }

      // 年份 + 类型 作为 genreTitle 前缀
      const yearStr = item.media_type === "movie" 
        ? (item.release_date || "").substring(0, 4)
        : (item.first_air_date || "").substring(0, 4);

      if (yearStr && widgetItem.genreTitle) {
        widgetItem.genreTitle = `${yearStr} • ${widgetItem.genreTitle}`;
      } else if (yearStr) {
        widgetItem.genreTitle = yearStr;
      }

      return widgetItem;
    }).slice(0, CONFIG.MAX_ITEMS);

    console.log(`✅ 主题分类加载成功: ${widgetItems.length} 条`);
    setCachedData(cacheKey, widgetItems);
    return widgetItems;

  } catch (error) {
    console.error("❌ 主题分类主流程失败:", error.message);
    return await loadThemeFallback_v2(params);
  }
}

// 主题分类回退函数 - 简化版
async function loadThemeFallback_v2(params = {}) {
  const {
    theme = "action",
    media_type = "all",
    sort_by = "popularity_desc",
  ...
  try {
    console.log("🔄 主题分类回退获取...");

    // 简化的单genre映射（仅电影）
    const SIMPLE_GENRE_MAP = {
    ...
    const genreId = SIMPLE_GENRE_MAP[theme];
    if (!genreId) {
      console.error(`❌ 回退也不支持的主题: ${theme}`);
      return [];
    }
    ...
    if (year) {
      queryParams["primary_release_date.gte"] = `${year}-01-01`;
      queryParams["primary_release_date.lte"] = `${year}-12-31`;
    }
    ...
    const results = res.results
      .filter(item => item.poster_path && item.title && !item.poster_path.includes("null"))
      .map(item => {
        item.media_type = "movie";
        const widgetItem = createWidgetItem(item);
        widgetItem.genreTitle = getGenreTitle(item.genre_ids, "movie");
        widgetItem.theme = theme;
        widgetItem.source = "TMDB主题分类(回退)";

        const yearStr = (item.release_date || "").substring(0, 4);
        if (yearStr && widgetItem.genreTitle) {
          widgetItem.genreTitle = `${yearStr} • ${widgetItem.genreTitle}`;
        } else if (yearStr) {
          widgetItem.genreTitle = yearStr;
    ...
    console.log(`✅ 主题分类回退成功: ${results.length} 条`);
    return results;

  } catch (error) {
    console.error("❌ 主题分类回退也失败:", error.message);
    return [];
  }
}

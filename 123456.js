    ...
    try {
        ...
        const promises = list.map(async (item, i) => {
            ...
            let finalItem = {
                ...
                mediaType: type,
                title: title,
                description: `${genreText}${dateDisplay ? ` (${dateDisplay})` : ''}`,
                releaseDate: releaseDate,
                posterPath: item.cover?.url || item.pic?.normal || "",
                backdropPath: item.pic?.large || item.cover?.url || "",
                genreTitle: genreText,
            ...
            try {
                ...
                if (tmdb) {
                    ...
                    finalItem.description = `${tmdbGenreText || genreText}${finalDateDisplay ? ` (${finalDateDisplay})` : ''}`;
                }
            } catch (tmdbError) {
        ...
        return results.map(item => { if (item) delete item.rating; return item; });

...
async function searchAndBlock(params = {}) {
  ...
  // 添加性能监控
  const endMonitor = performanceMonitor.start('TMDB搜索屏蔽模块');
  
  try {
    console.log(`🎯 执行操作: ${action}`);
    
    switch (action) {
      case "search_and_block":
        return await searchAndBlockContent(query, language);
      case "search_only":
        return await searchOnlyContent(query, language);
      case "manual_block":
        return await manualBlockContent(tmdb_id, media_type);
      default:
        console.log("⚠️ 未知操作类型");
        return [];
    }
    
  } catch (error) {
    console.error("❌ TMDB搜索屏蔽模块执行失败:", error);
    endMonitor();
    return [];
  } finally {
    endMonitor();
...
// 搜索并屏蔽内容
async function searchAndBlockContent(query, language) {
  if (!query || !query.trim()) {
    console.log("⚠️ 未提供搜索关键词");
    return [];
  }
  
  console.log(`🔍 搜索并屏蔽: "${query}"`);
  
  // 执行搜索
  const searchResults = await performTmdbSearch(query, "all", language, 1);
  ...
  console.log(`✅ 成功屏蔽 ${blockedItems.length} 项内容`);
  return blockedItems;
}

// 仅搜索内容
async function searchOnlyContent(query, language) {
  if (!query || !query.trim()) {
    console.log("⚠️ 未提供搜索关键词");
    return [];
  }
  
  console.log(`🔍 仅搜索: "${query}"`);
  ...
  const results = searchResults.map(item => ({
    ...item,
    isBlocked: isBlocked(item.id, item.mediaType),
    source: "TMDB搜索"
  }));
  
  console.log(`🔍 找到 ${results.length} 项内容`);
  return results;
}

// 手动屏蔽ID
async function manualBlockContent(tmdb_id, media_type) {
  if (!tmdb_id || !tmdb_id.trim()) {
    console.log("⚠️ 未提供TMDB ID");
    return [];
  }
  
  console.log(`🆔 手动屏蔽ID: ${tmdb_id} (${media_type})`);
  
  // 解析ID列表
  const ids = tmdb_id.split(',').map(id => id.trim()).filter(id => id);
  
  const blockedItems = [];
  for (const id of ids) {
    if (isValidId(id)) {
      // 获取详细信息
      const itemDetails = await getTmdbItemDetails(id, media_type);
      if (itemDetails) {
        const success = addToBlocklist(id, media_type, itemDetails.title);
        if (success) {
          blockedItems.push({
            id: id,
            title: itemDetails.title,
            mediaType: media_type,
            posterPath: itemDetails.posterPath,
...
async function manageBlockedItems(params = {}) {
  const { 
    action = "view", 
    unblock_id = "", 
    unblock_media_type = "tv", 
    import_data = "" 
  } = params;
  
  // 添加性能监控
  const endMonitor = performanceMonitor.start('TMDB屏蔽管理模块');
  ...
  try {
    ...
    switch (action) {
      case "view":
        return await viewBlockedItems();
      case "unblock":
        return await unblockItems(unblock_id, unblock_media_type);
      case "clear":
        return await clearBlockedItems();
      case "export":
        return await exportBlockedItems();
      case "import":
        return await importBlockedItems(import_data);
      default:
        console.log("⚠️ 未知操作类型");
        return [];
    }
    ...
    console.error("❌ TMDB屏蔽管理模块执行失败:", error);
    endMonitor();
    return [];
  } finally {
    endMonitor();
...
// 查看黑名单
async function viewBlockedItems() {
  try {
    const stored = Widget.storage.get(STORAGE_KEY);
    const blockedItems = stored ? JSON.parse(stored) : [];
    
    console.log(`📋 查看黑名单: ${blockedItems.length} 项`);
    ...
    console.error("❌ 查看黑名单失败:", error);
    return [];
  }
}

// 取消屏蔽
async function unblockItems(unblock_id, unblock_media_type) {
  if (!unblock_id || !unblock_id.trim()) {
    console.log("⚠️ 未提供要取消屏蔽的ID");
    return [];
  }
  ...
  try {
    ...
    const ids = unblock_id.split(',').map(id => id.trim()).filter(id => id);
    const unblockedItems = [];
    
    for (const id of ids) {
      const index = blockedItems.findIndex(item => 
        item.id === String(id) && item.media_type === unblock_media_type
      );
      
      if (index !== -1) {
        const removedItem = blockedItems.splice(index, 1)[0];
        unblockedItems.push({
          id: removedItem.id,
          title: removedItem.title,
          mediaType: removedItem.media_type,
          source: "取消屏蔽",
          type: "unblocked_item"
        });
      }
    ...
    console.log(`🔓 成功取消屏蔽 ${unblockedItems.length} 项内容`);
...
// 清空黑名单
async function clearBlockedItems() {
  try {
...
// 导出配置
async function exportBlockedItems() {
  try {
    const stored = Widget.storage.get(STORAGE_KEY);
    const blockedItems = stored ? JSON.parse(stored) : [];
    ...
    const exportData = {
      ...
      total_items: blockedItems.length,
      items: blockedItems
    };
    
    console.log(`📤 导出配置: ${blockedItems.length} 项`);
    
    return [{
      id: "export",
      title: `导出配置 (${blockedItems.length} 项)`,
      description: JSON.stringify(exportData, null, 2),
      source: "导出操作",
      type: "export_result"
    }];
...
// 导入配置
async function importBlockedItems(import_data) {
  if (!import_data || !import_data.trim()) {
    console.log("⚠️ 未提供导入数据");
    return [];
  }
  
  try {
    // 解析导入数据
    const ids = parseImportData(import_data);
    if (ids.length === 0) {
      console.log("⚠️ 导入数据格式错误");
      return [];
    }
    ...
    for (const idData of ids) {
      const { id, media_type = "movie" } = idData;
      
      // 检查是否已存在
      const exists = blockedItems.some(item => 
        item.id === String(id) && item.media_type === media_type
      );
      ...
      if (!exists) {
        ...
        if (itemDetails) {
          blockedItems.push({
            ...
            overview: itemDetails.overview || "通过导入添加的屏蔽项",
            blocked_date: new Date().toISOString(),
            vote_average: itemDetails.vote_average || 0
          });
    ...
    console.log(`📥 成功导入 ${importedItems.length} 项内容`);
...
function parseImportData(data) {
  ...
  // 解析逗号分隔的ID列表
  const idList = data.split(',').map(id => id.trim().replace(/['"]/g, '')).filter(id => id);
  
  for (const id of idList) {
    if (isValidId(id)) {
      ids.push({ id: String(id), media_type: "movie" });
    }
  }
...
// 获取TMDB项目详情
async function getTmdbItemDetails(id, media_type) {
  try {
    ...
    console.error(`❌ 获取TMDB详情失败 (${id}):`, error);
    return null;
  }
}

// 检查是否被屏蔽
function isBlocked(id, media_type) {
  return isItemBlocked({ id: id, media_type: media_type });
}

// 添加到黑名单（别名函数）
function addToBlocklist(id, media_type, title, additionalInfo = {}) {
  return addToBlockList(id, media_type, title, additionalInfo);
}

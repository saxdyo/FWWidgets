WidgetMetadata = {
  ...
  modules: [
    ...
    {
      title: "TMDB 今日热门",
      description: "今日热门电影与剧集，带logo背景图",
      requiresWebView: false,
      functionName: "loadTodayHotModule",
      cacheDuration: 3600,
      params: [
        {
          name: "media_type",
    ...
    {
      title: "TMDB 本周热门",
      description: "本周热门电影与剧集，带logo背景图",
      requiresWebView: false,
      functionName: "loadWeekHotModule",
      cacheDuration: 3600,
      params: [
        {
          name: "media_type",
    ...
    {
      title: "IMDB影视榜单",
      description: "使用本地抓取的IMDB数据",
      requiresWebView: false,
      functionName: "loadAutoFetchedData",
      cacheDuration: 3600,
      params: [
        {
          name: "data_type",
        ...
        },
        {
          ...
          enumOptions: [
            ...
            { title: "年份降序", value: "year_desc" },
            { title: "年份升序", value: "year_asc" },
            { title: "标题A-Z", value: "title_asc" },
            { title: "标题Z-A", value: "title_desc" }
          ]
        },
        {
          name: "page",
...
async function checkCdnHealth(cdnUrl) {
  const cacheKey = `cdn_health_${cdnUrl}`;
  const cached = cdnHealthCache.get(cacheKey);
  
  // 如果缓存时间小于5分钟，直接返回缓存结果
  if (cached && (Date.now() - cached.timestamp) < 5 * 60 * 1000) {
    return cached.healthy;
  }
  
  try {
    const testUrl = `${cdnUrl}/w92/test.jpg`;
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), CONFIG.CDN_CONFIG.HEALTH_CHECK.timeout);
    ...
    console.warn(`CDN健康检查失败: ${cdnUrl}`, error);
...
/**
 * 获取最佳CDN URL
 * @returns {Promise<string>} - 最佳CDN URL
 */
async function getBestCdnUrl() {
  // 如果健康检查未启用，直接返回主要CDN
  if (!CONFIG.CDN_CONFIG.HEALTH_CHECK.enabled) {
    return CONFIG.CDN_CONFIG.PRIMARY_CDN;
  }
  
  // 检查所有CDN的健康状态
  for (const cdnUrl of CONFIG.CDN_CONFIG.FALLBACK_CDNS) {
    const isHealthy = await checkCdnHealth(cdnUrl);
    if (isHealthy) {
      console.log(`✅ 使用CDN: ${cdnUrl}`);
      return cdnUrl;
    }
  }
  
  // 如果所有CDN都不可用，返回主要CDN
  console.warn("⚠️ 所有CDN都不可用，使用主要CDN");
  return CONFIG.CDN_CONFIG.PRIMARY_CDN;
}
 ...
 * @param {string} type - 图片类型 (poster/backdrop)
 * @returns {Promise<string>} - 优化的图片URL
 */
async function buildOptimizedImageUrl(imagePath, size = "large", type = "poster") {
  if (!imagePath) return "";
  
  // 获取最佳CDN
  const cdnUrl = await getBestCdnUrl();
  
  // 获取尺寸配置
  const sizeConfig = CONFIG.CDN_CONFIG.IMAGE_SIZES[type];
  const imageSize = sizeConfig[size] || sizeConfig.large;
  
  // 确保路径以/开头
  const normalizedPath = imagePath.startsWith('/') ? imagePath : `/${imagePath}`;
  
  return `${cdnUrl}/${imageSize}${normalizedPath}`;
}

/**
 * 批量构建优化的图片URL
 * @param {Object} item - 数据项
 * @returns {Promise<Object>} - 包含优化URL的对象
 */
async function buildOptimizedImageUrls(item) {
  ...
  // 构建海报URL
  if (item.poster_path) {
    const normalizedPath = item.poster_path.startsWith('/') ? item.poster_path : `/${item.poster_path}`;
    urls.posterPath = `${cdnUrl}/w500${normalizedPath}`;
    urls.coverUrl = urls.posterPath;
  }
  
  // 构建背景图URL
  if (item.backdrop_path) {
    const normalizedPath = item.backdrop_path.startsWith('/') ? item.backdrop_path : `/${item.backdrop_path}`;
    urls.backdropPath = `${cdnUrl}/w1280${normalizedPath}`;
    
    // 构建多种尺寸的背景图URL
    urls.backdropUrls = {
      w300: `${cdnUrl}/w300${normalizedPath}`,
      w780: `${cdnUrl}/w780${normalizedPath}`,
      w1280: `${cdnUrl}/w1280${normalizedPath}`,
      original: `${cdnUrl}/original${normalizedPath}`
    };
  }
...
async function createWidgetItem(item) {
  // 构建优化的图片URL
  const imageUrls = await buildOptimizedImageUrls(item);
...
async function loadTmdbTrendingWithAPI(params = {}) {
  ...
  try {
    ...
    let results = await Promise.all(response.results.map(async (item) => {
      ...
      // 使用优化的背景图
      if (item.backdrop_path) {
        const backdropUrl = await buildOptimizedImageUrl(item.backdrop_path, "large", "backdrop");
...
async function loadImdbAnimeModule(params = {}) {
  ...
  try {
    ...
    const widgetItems = rawData.map(item => {
...
async function loadTmdbByNetwork(params = {}) {
  ...
  try {
    ...
    const results = await Promise.all(res.results.map(async item => {
      ...
      return widgetItem;
    })).then(results => results.slice(0, CONFIG.MAX_ITEMS));
  ...
  try {
    ...
    if (type === "all") {
      ...
      const movieResults = await Promise.all(movieRes.results.map(async item => {
        ...
        return widgetItem;
      })).then(results => results.filter(item => item.posterPath));
...
async function loadTmdbByTheme(params = {}) {
  ...
  try {
    ...
    const results = await Promise.all(res.results.map(async item => {
      ...
      return widgetItem;
    })).then(results => results.filter(item => item.posterPath).slice(0, CONFIG.MAX_ITEMS));
...
function generateThemeFallbackData(theme) {
  ...
  const results = await Promise.all(fallbackData.map(async item => {
    const widgetItem = await createWidgetItem(item);
...
// 今日热门模块加载函数
async function loadTodayHotModule(params = {}) {
  const { media_type = "all", max_items = "20", language = "zh-CN" } = params;
  
  try {
    const cacheKey = `today_hot_module_${media_type}_${max_items}`;
    const cached = getCachedData(cacheKey);
    if (cached) return cached;

    console.log(`🔥 加载今日热门模块数据 (媒体类型: ${media_type}, 最大项目数: ${max_items})`);

    // 从预处理数据加载今日热门数据
    const response = await Widget.http.get("https://raw.githubusercontent.com/saxdyo/FWWidgets/main/data/TMDB_Trending.json");
    const data = response.data;
    
    let results = data.today_global || [];
    
    // 根据媒体类型过滤数据
    if (media_type !== "all") {
      results = results.filter(item => {
        const itemMediaType = item.type || (item.title ? "movie" : "tv");
        return itemMediaType === media_type;
      });
    }
    ...
    const widgetItems = await Promise.all(results.map(async (item) => {
      ...
      // 如果title_backdrop存在且包含logo生成服务，提取正常背景图作为备用
      if (item.title_backdrop && item.title_backdrop.includes('image-overlay.vercel.app')) {
        const normalBackdrop = item.title_backdrop.replace('https://image-overlay.vercel.app/api/backdrop?bg=', '').split('&')[0];
        widgetItem.normalBackdrop = normalBackdrop;
      }
    ...
    setCachedData(cacheKey, widgetItems);
    console.log(`✅ 今日热门模块加载成功: ${widgetItems.length}项`);
    return widgetItems;
    
  } catch (error) {
    console.error("今日热门模块加载失败:", error);
    return [];
  }
}

// 本周热门模块加载函数
async function loadWeekHotModule(params = {}) {
  const { media_type = "all", max_items = "20", language = "zh-CN" } = params;
  
  try {
    const cacheKey = `week_hot_module_${media_type}_${max_items}`;
    const cached = getCachedData(cacheKey);
    if (cached) return cached;

    console.log(`🔥 加载本周热门模块数据 (媒体类型: ${media_type}, 最大项目数: ${max_items})`);

    // 从预处理数据加载本周热门数据
    const response = await Widget.http.get("https://raw.githubusercontent.com/saxdyo/FWWidgets/main/data/TMDB_Trending.json");
    const data = response.data;
    
    let results = data.week_global_all || [];
    ...
    setCachedData(cacheKey, widgetItems);
    console.log(`✅ 本周热门模块加载成功: ${widgetItems.length}项`);
    return widgetItems;
...
// 自动抓取数据模块加载函数
async function loadAutoFetchedData(params = {}) {
  const { data_type = "movies", region = "all", sort_by = "popularity_desc", page = "1" } = params;
  
  try {
    const cacheKey = `auto_fetched_${data_type}_${region}_${sort_by}_${page}`;
    const cached = getCachedData(cacheKey);
    if (cached) return cached;

    console.log(`🎬 加载IMDB影视榜单数据 (类型: ${data_type}, 地区: ${region}, 排序: ${sort_by}, 页码: ${page})`);

    // 构建本地数据文件路径
    const cleanRegion = region.replace(':', '_');
    
    // 处理不同的排序类型
    let sortKey, dataPath;
    ...
    // 获取对应的数据文件排序键
    sortKey = sortMapping[sort_by] || 'hs';
    dataPath = `data/${data_type}/${cleanRegion}/by_${sortKey}/page_${page}.json`;
    ...
    function sortData(data, sortBy) {
      // 基础排序类型，数据已经预排序
      if (['popularity_desc', 'vote_average_desc'].includes(sortBy)) {
        return data;
      }
      ...
      switch (sortBy) {
        case 'popularity_asc': // 热度升序
          sortedData.sort((a, b) => (a.hs || 0) - (b.hs || 0));
          break;
          
        case 'vote_average_asc': // 评分升序
          sortedData.sort((a, b) => (a.r || 0) - (b.r || 0));
          break;
          
        case 'release_date_desc': // 上映时间降序
          sortedData.sort((a, b) => {
        ...
        case 'year_desc': // 年份降序
          sortedData.sort((a, b) => (b.y || 0) - (a.y || 0));
          break;
        ...
        case 'title_asc': // 标题A-Z
          sortedData.sort((a, b) => (a.t || '').localeCompare(b.t || '', 'zh-CN'));
          break;
          
        case 'title_desc': // 标题Z-A
          sortedData.sort((a, b) => (b.t || '').localeCompare(a.t || '', 'zh-CN'));
          break;
    ...
    const widgetItems = await Promise.all(sortedData.map(async item => {
      ...
      // 构建优化的图片URL
      const posterUrl = item.p ? await buildOptimizedImageUrl(item.p.startsWith('/') ? item.p : '/' + item.p, "large", "poster") : null;
      const backdropUrl = item.b ? await buildOptimizedImageUrl(item.b.startsWith('/') ? item.b : '/' + item.b, "medium", "backdrop") : null;
      ...
      // 确定媒体类型
      let mediaType = 'movie';
      if (data_type === 'tvseries' || data_type === 'anime') {
        mediaType = 'tv';
      }
